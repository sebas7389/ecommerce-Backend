...
function renderizarTabla(products) {
  ...
  if (products.length === 0) {
    tableBody.innerHTML =
      '<tr class="disable"> <td colspan = 6>NO SE ENCONTRARON PRODUCTOS </td></tr>';
    return;
  }

  products.forEach((producto) => {
    let imageSrc = producto.image? `/upload/product/${producto.image}`: "/assets/images/no-product.png";
    const tableRow = `
       <tr>
                ...
                <td class="admin-product__date">${formatDateAR(
                  producto.updateAt
                )}</td>
                <td >
                <div class = "admin-product__actions">
                <button class= "admin-product__delete-btn" onclick= "deleteProduct('${
                  producto._id
                }')" >
                <i class="fa-regular fa-trash-can"></i>
                </button>
                <button class= "admin-product__btn-edit" data-indice="${
                  producto._id
...
async function addProduct(evt) {
  try {
    ...
    if (editIndex) {
      ...
      if (!response) showAlert("No se pudo modificar el Producto", "error");
      else showAlert("Producto Actualizado Correctamente", "exito");
      ...
      if (!response) showAlert("No se pudo agregar el Producto", "error");
      else showAlert("Producto añadido Correctamente", "exito");
    }
...
function buscarProductos(text) {
  ...
  document.getElementById("admin-product-search-container_cant").innerText =
    "Cantidad de Coincidencias: " + cant;
